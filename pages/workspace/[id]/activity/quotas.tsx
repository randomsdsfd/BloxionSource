...
export const getServerSideProps = withPermissionCheckSsr(
  async ({ params }) => {
    ...
    if (!params?.id || isNaN(Number(params.id))) {
      return {
        redirect: {
          destination: "/workspace", // fallback if no ID
          permanent: false,
        },
      };
    }
    ...
    try {
      ...
      console.error("Error fetching quotas/roles:", err);
      return {
        notFound: true, // fallback if DB fails
      };
    }
    ...
    return {
      props: {
        quotas: JSON.parse(
          JSON.stringify(quotas, (_key, value) =>
...
const Quotas: pageWithLayout<pageProps> = (props) => {
  ...
  const [quotas, setQuotas] = useState<inactivityNotice[]>(
    props.quotas as inactivityNotice[]
  );
  ...
  const onSubmit: SubmitHandler<Form> = async ({
    ...
    const axiosPromise = axios
      .post(`/api/workspace/${id}/activity/quotas/new`, {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      Set requirements for your staff
                    </p>
                  </div>
                </div>
                <button
              ...
              {quotas.length === 0 ? (
                ...
                <div className="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
                  {quotas.map((quota: any) => (
                    <div
                      key={quota.id}
                      className="bg-gray-50 dark:bg-gray-500 rounded-lg p-4"
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button
                          onClick={() => {
                            const axiosPromise = axios
                              .delete(
                                `/api/workspace/${id}/activity/quotas/${quota.id}/delete`
                              )
                              .then((req) => {
                                setQuotas(
                                  quotas.filter((q: any) => q.id !== quota.id)
                                );
                              });
                            toast.promise(axiosPromise, {
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-white dark:bg-gray-800 p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title
                    as="h3"
                    className="text-lg font-medium text-gray-900 mb-4 dark:text-white"
                  >
                      ...
                      <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="space-y-4">
                          ...
                          <Input
                            ...
                            type="number"
                            append={
                              watch("type") === "mins" ? "Minutes" : "Sessions"
                            }
